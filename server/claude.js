...
function buildSystemPrompt({ allowedTopics, blockedTopics, includeMoralLesson }) {
  const lines = [
    ...
    'Dĺžka: približne 250-400 slov, jednoduchý jazyk, príjemný a pomalý záver, ktorý navodzuje spánok.',
  ];
  ...
  if (includeMoralLesson) {
    lines.push(
      'Do tejto rozprávky prirodzene zakomponuj jemné mravné ponaučenie, ktoré vyplynie z príbehu - nemá pôsobiť ako moralizovanie, ale ako prirodzená súčasť deja a záveru.'
    );
  }
...
async function generateStory({ childPrompt, allowedTopics, blockedTopics, includeMoralLesson }) {
  const system = buildSystemPrompt({ allowedTopics, blockedTopics, includeMoralLesson });
